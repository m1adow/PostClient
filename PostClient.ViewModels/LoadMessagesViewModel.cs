...
using PostClient.BackgroundTasks;
using System;
...
#nullable enable
    ...
    public class LoadMessagesViewModel : ViewModelBase
    {
        ...
        public LoadMessagesViewModel(Func<IPostService> getService, Func<Account> getAccount)
        {
        ...
        private async void LoadMessagesFromLocalStorage(object parameter)
        {
            try
            {
                string prefix = _getAccount().Email + _getAccount().PostServiceName;
                _messageFolder = prefix + parameter.ToString() + ".json";
        ...
        private async void LoadMessagesFromServer(object parameter)
        {
            ...
            try
            {
                ...
                SaveMessages(popMailMessages, $"{_getAccount().Email}{_getAccount().PostServiceName}POPMessages.json");
            }
            catch { }
        ...
        private async Task<ObservableCollection<MailMessage>> GetMessagesAsync()
        {
            ...
            string prefix = _getAccount().Email + _getAccount().PostServiceName;
            SaveMessages(allMailMessages, prefix + "AllMessages.json");
            SaveMessages(sentMailMessages, prefix + "SentMessages.json");
            SaveMessages(flaggedMailMessages, prefix + "FlaggedMessages.json");
            SaveMessages(new List<MailMessage>(), prefix + "DraftMessages.json"); //clear draft messages after syncing

            _messageFolder = prefix + "AllMessages.json";
            return messages;
        }
